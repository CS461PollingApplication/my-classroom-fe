...
function Enrollments(props) {
    ...
    return(
        ...
        <ul className="allstudents">
            { (loading) ? <TailSpin visible={true}/> : enrollments[courseId] && enrollments[courseId].filter(studentInSection).length > 0 ? enrollments[courseId].filter(studentInSection).map((enrollment) => {
